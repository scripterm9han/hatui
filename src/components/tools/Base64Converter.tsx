...
export default function Base64Converter() {
  ...
  const handleConvert = () => {
    ...
    try {
      ...
      setError(mode === "decode" ? "Invalid Base64 character sequence." : err.message);
      setOutput("");
    }
  };
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetch("/api/tools/log-usage", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ toolSlug: "base64-converter" }),
        });
      } else {
        setSaveStatus("error");
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col space-y-2">
          <label className="text-slate-400 text-xs font-mono uppercase tracking-wider">
            {mode === "encode" ? "Source Raw Text" : "Base64 Input"}
          </label>
          <textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder={mode === "encode" ? "Enter raw text to encode..." : "Enter Base64 text to decode..."}
